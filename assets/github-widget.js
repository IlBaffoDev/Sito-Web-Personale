  ...
  fetch('https://api.github.com/users/IlBaffoDev/repos?sort=updated&per_page=6&type=owner')
    ...
    .then(function (repos) {
      ...
      var cards = list.map(function (repo) {
        ...
        meta.push(new Date(repo.updated_at).toLocaleDateString('it-IT'));
        return (
      ...
      section.innerHTML =
        '<p style="font-family:\'IBM Plex Mono\',monospace;font-size:13px;letter-spacing:.08em;text-transform:uppercase;color:#4FE3C1;margin:0 0 12px">Attività</p>' +
        '<h2 style="font-family:\'Space Grotesk\',sans-serif;font-weight:600;font-size:30px;letter-spacing:-.3px;margin:0 0 32px">Cosa sto costruendo su GitHub.</h2>' +
